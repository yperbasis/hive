...
// setClientPoolKey attaches a pool key to a registered ClientInfo so that
// EndTest / StopNode can route the container to the pool instead of deleting
// it. No-op if the test or node can't be found.
func (manager *TestManager) setClientPoolKey(testID TestID, nodeID, key string) {
	manager.testCaseMutex.Lock()
	defer manager.testCaseMutex.Unlock()
	tc, ok := manager.runningTestCases[testID]
	if !ok {
		return
	}
	if info, ok := tc.ClientInfo[nodeID]; ok {
		info.poolKey = key
	}
}
